...
// Zig 0.15.2 can crash while parsing Mach-O unwind metadata emitted for some
// darwin/aarch64 s2n-bignum assembly objects. Preserve the arm64 assembly
// implementation but suppress CFI directives for this configuration.
#if defined(__APPLE__) && defined(__aarch64__)
#define CFI_START /* no cfi */
#define CFI_RET ret
...
#define CFI_STACKSAVE2X(lo,hi,offset,offset8) stp     lo, hi, [sp, #(offset+0)]
#define CFI_STACKSAVE1Z(reg,offset) stp     reg, xzr, [sp, #(offset+0)]

#define CFI_STACKLOAD2(lo,hi,offset) ldp     lo, hi, [sp, #(offset+0)]
#define CFI_STACKLOAD1Z(reg,offset) ldp     reg, xzr, [sp, #(offset+0)]

#define CFI_INC_SP(offset) add     sp, sp, #(offset+0)
#define CFI_DEC_SP(offset) sub     sp, sp, #(offset+0)
#else
