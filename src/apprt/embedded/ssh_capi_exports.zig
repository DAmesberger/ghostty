//! C-ABI export functions for the libghostty SSH connection + channel
//! multiplexing surface, plus the setup worker and the full inline test
//! suite. Split out of `ssh_capi.zig`.
//!
//! EVERY `export fn` lives here. An `export fn` only emits its C symbol
//! when its file is pulled into the PRODUCTION compilation graph, so the
//! facade force-imports this file with a top-level
//! `comptime { _ = @import("ssh_capi_exports.zig"); }` — a test-only
//! reference would silently drop the C symbols and break the cmux link.
...
// Runtime handles + translation/resolution helpers.
const handles = @import("ssh_capi_handles.zig");
const SshHandle = handles.SshHandle;
const ChannelHandle = handles.ChannelHandle;
const OwnedConfig = handles.OwnedConfig;
const translateState = handles.translateState;
const managerFromAppPtr = handles.managerFromAppPtr;
const protocolServiceFromC = handles.protocolServiceFromC;
...
const SessionStatus = types.SessionStatus;
const SessionEntry = types.SessionEntry;
...
/// `ClientMux.writeChannel` (credit-bounded — 0 means "no credit,
/// wait for on_window_credit"). SIZE_MAX signals a terminal error: a
/// closed channel, a channel that never reached a live mux, or an
/// unknown channel id.
...
/// on_close — the embedder initiated this), then fires the embedder's
/// on_close locally via emitClose so the C API contract ("close
/// triggers an on_close callback") still holds. emitClose is
/// idempotent, so a racing daemon-side close is harmless.
