...
// =========================================================================
// Viewport diff application (client side)
// =========================================================================

/// Apply a binary page diff to the client's terminal viewport.
/// This is the client-side counterpart to serializeDirtyRows/serializeFullSnapshot.
/// Directly writes cells into the terminal's page memory — no VT parsing.
pub fn applyPageDiff(t: *Terminal, data: []const u8) void {
    if (data.len < header_size) return;
    var offset: usize = 0;

    // Parse header
    const diff_type_byte = data[offset];
    offset += 1;
    _ = diff_type_byte; // incremental vs full_snapshot — apply logic is the same

    // seq (skip for now)
    offset += 2;

    // Cursor position
    const cursor_y = std.mem.readInt(u16, data[offset..][0..2], .little);
    offset += 2;
    const cursor_x = std.mem.readInt(u16, data[offset..][0..2], .little);
    offset += 2;

    // Cursor style + blink
    const cursor_info = decodeCursorStyle(data[offset]);
    offset += 1;

    // Cursor visible
    const cursor_visible = data[offset] != 0;
    offset += 1;

    // Mode flags
    const mode_flags = data[offset];
    offset += 1;

    // Dirty row count
    const dirty_row_count: u16 = data[offset];
    offset += 1;

    // Scan past rows to find the style table at the end.
    var scan_offset = offset;
    for (0..dirty_row_count) |_| {
        if (scan_offset + row_overhead > data.len) return;
        scan_offset += 2; // row_index
    ...
    var actual_style_count: usize = 0;

    if (scan_offset + 2 <= data.len) {
        const style_count = std.mem.readInt(u16, data[scan_offset..][0..2], .little);
        scan_offset += 2;
        actual_style_count = @min(style_count, max_inline_styles);
        for (0..actual_style_count) |i| {
            if (scan_offset + 2 > data.len) break;
            server_ids[i] = std.mem.readInt(u16, data[scan_offset..][0..2], .little);
            scan_offset += 2;
            const result = deserializeStyle(data[scan_offset..]);
            server_styles[i] = result.s;
            scan_offset += result.len;
        }
    }

    // Apply rows to viewport
    const s: *Screen = t.screens.active;
    for (0..dirty_row_count) |_| {
        if (offset + row_overhead > data.len) return;

        const row_index = std.mem.readInt(u16, data[offset..][0..2], .little);
        offset += 2;
        ...
        // Pin the viewport row
        const pin = s.pages.pin(.{ .viewport = .{
            .x = 0,
            .y = row_index,
        } }) orelse {
            offset += @as(usize, cell_count) * 8;
            continue;
        };
        ...
        for (0..write_count) |ci| {
            ...
            if (cell.style_id != 0) {
                ...
                for (0..actual_style_count) |si| {
                    if (server_ids[si] == cell.style_id) {
                ...
                if (found_style) |fs| {
                    const client_id = p.styles.add(p.memory, fs) catch |err| blk: {
                        switch (err) {
                            error.NeedsRehash, error.OutOfMemory => {},
                        }
            ...
            // Clear grapheme tag (viewport diffs don't include grapheme data)
        ...
        for (write_count..@as(u16, @intCast(page_cells.len))) |ci| {
            page_cells[ci] = .{};
        }

        // Skip remaining cell data if cell_count > page width
    ...
    // Apply cursor position and modes
    t.screens.active.cursor.x = @min(cursor_x, if (t.cols > 0) t.cols - 1 else 0);
    t.screens.active.cursor.y = @min(cursor_y, if (t.rows > 0) t.rows - 1 else 0);
    t.screens.active.cursor.cursor_style = cursor_info.style;
    t.modes.set(.cursor_blinking, cursor_info.blink);
    t.modes.set(.cursor_visible, cursor_visible);
    t.modes.set(.origin, (mode_flags & 0x02) != 0);
    t.modes.set(.wraparound, (mode_flags & 0x04) != 0);
}
